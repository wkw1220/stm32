...
void Init_InternelFun()
{
  ...
  /* 2.配置GPIO引脚的工作模式 PC10=TX(输出复用推挽CNF=10 MODE=11) PC11=RX(浮空输入 CNF=01 MODE=00)  */
  ...
  /* 3.串口的参数配置 */
  /* 3.1 配置波特率 9600 */
  USART3->BRR = 0x1D4C;//  0x1D4C 0x0271
  /* 3.2 配置一个字的长度 8位 */
...
void Dri_USART_SendByte(uint8_t oneByte)
{
  ...
  /**
   * @brief 一直等到发送寄存器为空
   * while (!(USART3->SR & USART_SR_TXE)); 和下面语句等同
   */
...
void Dri_USART_ReceiveStr(u8 *buff)
{
  ...
  {
    // 判断寄存器中是否有数据，有数据就读取
    while ((USART3->SR & USART_SR_RXNE) != 0) // 0 没有收到数据 1 收到数据 可以读取
    {
      buff[i] = USART3->DR;
      i++;
    }
