 ...
 *  
 *  以前的电脑CPU有个专门的插槽，GPU(显卡)有个专门的插槽，内存(单片机的RAM)，硬盘(单片机的Flash)也分别对应的有插槽。但是随着集成化
 *  越来越高，ARM体系的CPU把这些功能模块全部集成起来放在一起了，从外观上是看不出来的，现在所讲的CPU其实
 *  就是片上外设SoC 
 *   CPU是arm公司设计的，生产厂家有很多（st公司，苹果等）
 * 
 *   地址总线：AB  控制总线:CB 数据总线：DB
 *   说这个芯片是多少位，一般指的是地址是多少位
 *   stm32 指的是地址是32位，寻址空间是2^32=4G的空间
 * 
 *   ARM出图纸
 ...
 *   新的指令集:Risc-V
 * 
 *   Cortex-A系列：主打高性能。例如手机 平板 智能电视
 *   Cortex-R系列：主打实时 例如：汽车 工业控制 医疗设备
 *   Cortex-M系列：主打嵌入式 例如传感器 智能家居等
 * 
 */
 ...
 * type-c的供电方式
 *   5v------->ASM1117----->3.3v
 *   最低压差为1.2V，也就是说最少为4.5V
 *   万一提供的电压小于4.5V会发生什么？
 *   会导致供电电压一直往下掉，使得芯片不能正常工作
 * 
 */
...
/**
 * 电容具有滤波功能
 * 电容容量越大滤波半径越大，电容容量越小滤波半径越小  给芯片供电时一般在供电引脚接一个大电容
 * 和小电容，一般小电容里引脚近，大电容一般和电源供电接近
 * 现代单片机一般都是多引脚供电
 * 像51单片机一般都是单引脚供电
 * 
 * 简牛：加了黑色套的排针 可以淘宝搜索图片
 * 
 * NRST:Negative Reset 低电平复位
 * Keil 开发软件 也叫MDK ARM,其实是同一个软件
 */
