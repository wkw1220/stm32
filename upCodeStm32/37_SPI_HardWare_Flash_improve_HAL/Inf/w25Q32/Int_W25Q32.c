...
void Inf_W25Q32_ReadId(uint8_t *mid, uint16_t *did)
{
    ...
    uint8_t buff[3]={0};
    HAL_SPI_Receive(&hspi1,buff,3,2000);
...
void Inf_W25Q32_WaitNotBusy(void)
{
    ...
    uint8_t r;
    do{
      HAL_SPI_Receive(&hspi1,&r,1,2000);
    }while(r&0x01);
...
void Inf_W25Q32_EraseSector(uint8_t block, uint8_t sector)
{
    ...
    uint8_t buff[3]={sectorAddr >> 16 & 0xff,sectorAddr >> 8 & 0xff,sectorAddr & 0xff};
    HAL_SPI_Transmit(&hspi1,buff,3,2000);
...
void Inf_W25Q32_WritePage(uint8_t block, uint8_t sector, uint8_t page, uint8_t *data, uint16_t len)
{
    ...
    uint8_t buff[3]={pageAddr >> 16 & 0xff,pageAddr >> 8 & 0xff,pageAddr & 0xff};
    HAL_SPI_Transmit(&hspi1,buff,3,2000);
