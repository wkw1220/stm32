...
void Driver_SysTick_Init(void)
{
    ...
    /** 
     *  4使能定时器 
     *  既然系统定时器是内核的，整个系统都依赖，而且从HAL中能看到是默认打开且不能更改的
     *  但是为什么这里还能配置使能呢，默认不是开启的吗？如果关闭了又会发生什么？
     *  而且还能配置中断位，这些不会影响系统吗？
     *  
     *   HAL里配置是NVIC里的中的滴答定时器的开启使能(默认就是开启的)
     *   还有就是通过 SysTick->CTRL 中的SysTick_CTRL_TICKINT  
     *   也就是：开启滴答定时器的中断需要配置两个地方
     * */
