 ...
 *  例如按下按键PA0,让串口发送数据
 *   信号中断机制:PA10会利用AFIO产生中断,然后传递给EXTI,然后再经过NVIC到CPU,CPU调用中断
...
void Dri_Key_Init_PF10(void)
{
  ...
  /** 1.2 AFIO It will start the function of AFIO with the function of external interrupt */
  RCC->APB2ENR |= RCC_APB2ENR_AFIOEN;//我要用复用功能,到底复用的是啥功能,外部中断功能,所以后续在AFIO里配置外部中断
   ...
   * 下标是0到3,所以下面代码下标是2
   * 
   * tell the AFIO to connect the PF10 to the EXTI10,will use the EXTI10 to trigger the interrupt
   */
  //先把这四位清零 然后再或
...
void Dri_Key_Init_PF11(void)
{
  ...
  /** 2.配置PF11 的输入模式 MODE:输入模式00 CNF CNF:10(上拉或者下拉) ODR:0下拉 1:上拉 */
  GPIOF->CRH &= ~GPIO_CRH_MODE11;
  GPIOF->CRH |= GPIO_CRH_CNF11_1;
  GPIOF->CRH &= ~GPIO_CRH_CNF11_0;
  GPIOF->ODR &= ~GPIO_ODR_ODR11;//下拉
...
void Dri_Key_Init_PF9(void)
{
   ...
   *
   * 要启用引脚的中断服务功能
   *3. 配置AFIO 配置PF9引脚为外部中断  EXTICR3 0101
   *EXTICR有四个,序号是 1 2 3 4 但是AFIO->EXTICR用的是数组实现
   *下标是0到3,所以下面代码下标是2
   */
  // 先把这四位清零 然后再或 PF  0101 可以先统一设置为0 再设置为0101
  ...
  /* 4.2 开启 LINE9, 配置的中断屏蔽寄存器,因为是与门连接的NVIC */
...
void EXTI15_10_IRQHandler(void)
{
  ...
  }else if (EXTI->PR & EXTI_PR_PR11)
  {
    ...
    EXTI->PR = EXTI_PR_PR11; // 这样写的效率更高,查看手册
    
    Delay_ms(10); // 10到15ms消抖

    // 判断按键是否是真的按下
    if ((GPIOF->IDR & GPIO_IDR_IDR11) != 0)//稳定后引脚是3.3v
    {
      Dri_Led_Toggle(LED2);
    }
  }
