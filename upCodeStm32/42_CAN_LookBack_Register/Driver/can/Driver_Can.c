...
/**
 * @brief 工作流程详解
 * 1.报文信息先经过过滤器过滤-->进入FIFO队列(两个队列)(一个队列可以存放3个报文，也就是队列的长度
 *   为3，也叫邮箱)
 *
 *  过滤器组:互联网有28组(0-27),其它产品为0-13共14组，随便选择一组，默认从零开始
 *      过滤器模式：
 *      1.白名单模式(标识符列表模式)：配置ID的方式，配置谁，谁进入
 *      2.黑名单模式(掩码模式): 配置黑名单模式规则，符合黑名单模式的不准进入，不符合黑名单模式的进入
 *      一个组里有2个32位的寄存器FR1，FR2(可以配置2个白名单)，也就是是4个16位(可以配置4个白名单，用16位位移的方式实现)
 *      FR1：基准ID,FR2:掩码规则
 * 
 * 报文通过了过滤器组就开始进入FIFO队列了
 *  有两个队列选择进入哪个队列
 */
void Driver_Can_FilterConfig(void)
{
    ...
    /* 4.7.2 过滤器组0为单个32位配置 0:2个16位 1:单个32位*/
...
void Driver_Can_Init(void)
{
    ...
    /**2.给引脚CanTx和CanRx配置工作模式 给can的Rx和Tx重映射到PB8和PB9 新知识点:重映射功能*/
