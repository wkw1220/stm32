 ...
 *
 * ADC模块其实就是一个电压表
 * 在引脚输入模式中，有模拟输入，模拟输入至片上外设，如何读取模拟值，
 * ADC实际上接的是一个电压表,ADC可以直接测量引脚的电压
 *
 * 在输入模式中 有上拉 下拉 浮空和模拟输入。在外部传感器可以推挽输入的时候我们选择浮空输入，
 * 如果外部传感器是开漏输入，要么是高组态，要么是零低电平
 * 我们可以选择上拉输入，在传感器高组态的时候保持输入高电平。一般很少用到下拉输入。如果是和上拉相反。外部传感器高组态时保持低电平。但是这种情况非常少见
 * 
 * 在浮空输入模式下，施密特触发器会导通，但是模拟输入照样导通，也就是会同时产生模拟量和数字量（在ADC模块可以进行测量）
 */
...
/**
 * 引脚输出电平可以用ODR，读取引脚电平用IDR，但是发现在推挽输出模式下用ODR也能获取电平引脚电压
 *  ODR既可以写 也可以读取，要根据实际情况分析
 */
 ...
 * 
 *        BSRR  BR位=1，输出低电平会使灯亮 BS=1输出高电平位会使灯灭 如果BS和BR同时设置为1，BS起作用，也就是灯是灭的  最终影响的是ODR
 *        GPIOE ->BSRR|=GPIO_BSRR_BS5;或者这样写 GPIOE ->BSRR=GPIO_BSRR_BS5；用后一种写法，因为根据手册0对对应的ODRy位不产生影响
 *        BSRR寄存器只能写，不能读取。
 *        BRR现在已经不用了，是为了兼容以前的旧版本
 * 
 * @param br 
 */
